            ...
            with torch.no_grad():
                ...
                if self.use_fusion:
                    if len(passage_embeddings)>1:
                        final_embedding = self.fusion_network(passage_embeddings)
                    else:
                        final_embedding = passage_embeddings[0]
                else:
                    if len(passage_embeddings)>1:
                        #multi
                        stacked = torch.cat(passage_embeddings, dim=0)  # [num_docs, 2048]
                        final_embedding = stacked.mean(dim=0, keepdim=True)  # [1, 2048]
                    else:
                        # 单文档: 直接使用
...
def main(args):
    ...
    for epoch in range(DYPRAG_TRAIN_EPOCH):
        ...
        save_dir = os.path.join(ROOT_DIR, "projector", f'{args.model_name}_hidden{args.projector_p}_sample{args.sample_rate}_lr{args.dyprag_learning_rate}_embedding_self_attention')
