...
def get_train_data(aug_model, augments, tokenizer, args):
    ...
    for aug in augments:
        ...
        if "is_multi_doc" in aug:
            is_multi_doc = aug['is_multi_doc']
        else:
            is_multi_doc = False
        if is_multi_doc:
            psg = psg.split(" [sep] ")
...
def main(args):
    if args.projector:
        print("projector:args.projector")
        data_list = load_data(args.dataset, args.data_type, args.augment_model, data_dir="./data_aug_projector")
    else:
