# Stage 2 (Phase 2): Train Projector
import os
import gc
import json
import argparse
...
class TrainingDataCollator(DefaultDataCollator):
    ...
    def __call__(self, examples: List[Dict[str, dict]]) -> Dict[str, torch.Tensor]:
        ...
        for example in examples:
            ...
            with torch.no_grad():
                output = self.model(tokenized.input_ids, tokenized.attention_mask, output_hidden_states=True)
...
def main(args):
    ...
    for epoch in range(DYPRAG_TRAIN_EPOCH):
        for step, batch in enumerate(train_dataloader):
            ...
            print(f"Stage 2.1: Epoch {epoch}, Step {step}, MSE Loss: {mse_loss.item():.4f}, LM Loss: {lm_loss.item():.4f}, KL Loss: {kl_loss.item():.4f}, Target LM Loss: {lm_origin_loss.item():.4f}")
        save_dir = os.path.join(ROOT_DIR, "projector", f'{args.model_name}_hidden{args.projector_p}_sample{args.sample_rate}_lr{args.dyprag_learning_rate}_stage1')
